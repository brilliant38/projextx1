package com.projectx3.image.mapper;
...
public interface ImageMapper {
	...
	public int write(ImageVO vo); // 먼저 시퀀스에서 글 번호를 받아서 vo.no에 세팅 처리해 주게 한다. select key 태그 이용
	public int writeFile(ImageFileVO vo);
	
	//4. 이미지 수정 - 텍스트 정보 수정, 이미지 파일 수정, 대표 이미지 변경
	public int update(ImageVO vo);
	public int updateFile(ImageFileVO vo);
	public int updateFileUnPresident(long no);
	public int updateFilePresident(long fno);
